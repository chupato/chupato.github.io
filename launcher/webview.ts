...
const dlls = await getDll()

try {
  await Deno.rename(dlls.WebView2Loader, `./WebView2Loader.dll`)
} catch {
  // ignore errors, probably fine
}

const instances: Webview[] = []
const encoder = new TextEncoder()
const c_str = (str: string) => encoder.encode(str + '\0')

const lib = Deno.dlopen(dlls.webview, {
  ...
  webview_set_html: { parameters: ['pointer', 'pointer'], result: 'void' },
  ...
  webview_unbind: { parameters: ['pointer', 'buffer'], result: 'void' },
  webview_return: {
...
/** Window size hints */
type SizeHint = typeof SizeHint[keyof typeof SizeHint]
...
export class Webview {
  ...
  set title(title: string) {
    lib.symbols.webview_set_title(this.#handle, c_str(title))
  }
   ...
   * `"data:text/html,<html>...</html>"`. It is often ok not to url-c_str it
   * properly, webview will re-c_str it for you.
   */
  navigate(url: URL | string) {
    lib.symbols.webview_navigate(
      this.#handle,
      c_str(url instanceof URL ? url.toString() : url),
    )
  }
   ...
   */
  unbind(name: string) {
    lib.symbols.webview_unbind(this.#handle, c_str(name))
   ...
   */
  eval(source: string) {
    lib.symbols.webview_eval(this.#handle, c_str(source))
  }
   ...
   */
  init(source: string) {
    lib.symbols.webview_init(this.#handle, c_str(source))
  }
}
